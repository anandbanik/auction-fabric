...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Auction House Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-auctionhouse.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create Authenticator Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-authenticator.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
    ...
    echo "##########################################################"
    echo "############ Create Appraiser Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-appraiser.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  echo
  echo "Generate CCP files for all Peer Organizations"
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile ArtNftOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
# This method is starting a blockchain explorer instance
...
function startMetricServer() {
  ...
  docker-compose -f ${COMPOSE_FILE_METRICS} up -d 2>&1
...
## Bring up the network if it is not arleady up.
 ...
 scripts/createArtNftChannels.sh $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
...
function deployCC() {
  ...
  scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_RUNTIME_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE $CC_END_POLICY $CC_COLL_CONFIG
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_API -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_METRICS down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf system-genesis-block/*.block organizations/peerOrganizations organizations/ordererOrganizations credentials/wallet/*.id
    ## remove fabric ca artifacts
    rm -rf organizations/fabric-ca/auctionhouse/msp organizations/fabric-ca/auctionhouse/tls-cert.pem organizations/fabric-ca/auctionhouse/ca-cert.pem organizations/fabric-ca/auctionhouse/IssuerPublicKey organizations/fabric-ca/auctionhouse/IssuerRevocationPublicKey organizations/fabric-ca/auctionhouse/fabric-ca-server.db
    rm -rf organizations/fabric-ca/authenticator/msp organizations/fabric-ca/authenticator/tls-cert.pem organizations/fabric-ca/authenticator/ca-cert.pem organizations/fabric-ca/authenticator/IssuerPublicKey organizations/fabric-ca/authenticator/IssuerRevocationPublicKey organizations/fabric-ca/authenticator/fabric-ca-server.db
    rm -rf organizations/fabric-ca/appraiser/msp organizations/fabric-ca/appraiser/tls-cert.pem organizations/fabric-ca/appraiser/ca-cert.pem organizations/fabric-ca/appraiser/IssuerPublicKey organizations/fabric-ca/appraiser/IssuerRevocationPublicKey organizations/fabric-ca/appraiser/fabric-ca-server.db
...
# use this for the API server
COMPOSE_FILE_API=docker/docker-compose-api.yaml
# use this for creating kafka service
COMPOSE_FILE_KAFKA=docker/docker-compose-kafka.yaml
# Use this for metric services like explorer, grafana and prometheus
COMPOSE_FILE_METRICS=docker/docker-compose-metrics.yaml
#
...
if [ "$MODE" == "up" ]; then
  ...
  echo "Generating certificats for Orgs"
  echo 
elif [ "${MODE}" == "api" ]; then
  echo "Generating wallet and provision REST service"
  echo
